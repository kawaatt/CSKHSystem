    ...
    public class BaseServices : IBaseServices
    {
        ...
        public async Task<ResponseDTO?> LiveSendFormURLEncodeContentAsync(RequestDTO requestDTO, bool withBearer = true)
        {
            try
            {
                ...
                if (withBearer)
                {
                    userInfor = await _userContextService.GetUserInfoAsync();
                    if (userInfor.AccessToken == null)
                    {
                        throw new ArgumentNullException("null AccessToken");
                    }
                    message.Headers.Add("Authorization", $"Bearer {userInfor.AccessToken}");
                }
